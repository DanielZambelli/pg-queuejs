...
class PgTaskQue{
  ...
  async que(task_id, payloads, awaitCompleted=false, returnRows=false, awaitCompletedMs=1000){
    ...
    if(awaitCompleted) {
      await this.completedAll(task_id, awaitCompletedMs)
      if(returnRows) return await this.rows.findAll({ where: { id: ids } })
    }
    return ids
  }
  ...
  async _getRows(map){
    ...
    const limit = map.find(obj => obj.task_id === task_id)?.batch || 1
      ...
      while(true){
        ...
        await Promise.all(rows.map(row => {
          return new Promise(async (resolve) => {
            try{
              row.result = await map.find(obj => obj.task_id === row.task_id).callback(row.payload)
              row.status = 'completed'
            }catch(err){
              row.status = 'error'
              row.log = err.toString().substring(0, 254)
            }finally{
              await row.save()
